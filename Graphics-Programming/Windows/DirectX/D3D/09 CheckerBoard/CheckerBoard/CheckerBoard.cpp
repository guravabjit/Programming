...
byte ggluCheckImage[CHECK_IMAGE_WIDTH][CHECK_IMAGE_HEIGHT][4];
...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_TiltedQuad = NULL;
...
HRESULT initialize()
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_TiltedQuad);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
	...
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "ID3D11Device::CreateBuffer() Failed For Vertex Buffer for tilted quad.\n");
		fclose(gpLogFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "ID3D11Device::CreateBuffer() Succeeded For Vertex Buffer for tilted quad.\n");
		fclose(gpLogFile);
	}

	D3D11_MAPPED_SUBRESOURCE mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Quad,
		NULL,
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_TiltedQuad,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresource);
	memcpy(mappedSubresource.pData, verticesTiltedQuad, sizeof(verticesTiltedQuad));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_TiltedQuad, NULL);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Quad_Texture,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresource);
	...
	hr = LoadD3DTexture(&gpID3D11ShaderResourceView_Quad_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
...
void display()
{
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_TiltedQuad, &stride, &offset);
	stride = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Quad_Texture, &stride, &offset);
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Quad_Texture);
...
HRESULT LoadD3DTexture(ID3D11ShaderResourceView ** ppID3D11ShaderResourceView)
{
	...
	D3D11_TEXTURE2D_DESC checkerBoardTextureDesc;
	ID3D11Texture2D* checkerBoardTexture = NULL;
	D3D11_SHADER_RESOURCE_VIEW_DESC checkerBoardResourceViewDesc;

	MakeCheckerImage();
	ZeroMemory(&initData, sizeof(D3D11_SUBRESOURCE_DATA));
	initData.pSysMem = ggluCheckImage;
	initData.SysMemPitch = CHECK_IMAGE_WIDTH * 4;

	ZeroMemory(&checkerBoardTextureDesc, sizeof(D3D11_TEXTURE2D_DESC));
	checkerBoardTextureDesc.Width = CHECK_IMAGE_WIDTH;
	checkerBoardTextureDesc.Height = CHECK_IMAGE_HEIGHT;
	checkerBoardTextureDesc.MipLevels = 1;
	checkerBoardTextureDesc.ArraySize = 1;
	checkerBoardTextureDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	checkerBoardTextureDesc.SampleDesc.Count = 1;
	checkerBoardTextureDesc.SampleDesc.Quality = 0;
	checkerBoardTextureDesc.Usage = D3D11_USAGE_DEFAULT;
	checkerBoardTextureDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET;
	checkerBoardTextureDesc.CPUAccessFlags = 0;
	checkerBoardTextureDesc.MiscFlags = 0;

	hr = gpID3D11Device->CreateTexture2D(&checkerBoardTextureDesc, &initData, &checkerBoardTexture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "CreateTexture2D() failed for checkerBoardTexture.\n");
		fclose(gpLogFile);
		return(hr);
	}

	memset(&checkerBoardResourceViewDesc, 0, sizeof(checkerBoardResourceViewDesc));
	checkerBoardResourceViewDesc.Format = checkerBoardTextureDesc.Format;
	checkerBoardResourceViewDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	checkerBoardResourceViewDesc.Texture2D.MostDetailedMip = 0;
	checkerBoardResourceViewDesc.Texture2D.MipLevels = 1;

	hr = gpID3D11Device->CreateShaderResourceView(checkerBoardTexture, &checkerBoardResourceViewDesc, ppID3D11ShaderResourceView);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "CreateShaderResourceView() failed for checkerBoardResourceViewDesc.\n");
		fclose(gpLogFile);
		return(hr);
	}
...
void MakeCheckerImage()
{
	...
	for (i = 0; i < CHECK_IMAGE_WIDTH; i++)
	{
		for (j = 0; j < CHECK_IMAGE_HEIGHT; j++)
		{
			...
			ggluCheckImage[i][j][0] = (byte)c;
			ggluCheckImage[i][j][1] = (byte)c;
			ggluCheckImage[i][j][2] = (byte)c;
