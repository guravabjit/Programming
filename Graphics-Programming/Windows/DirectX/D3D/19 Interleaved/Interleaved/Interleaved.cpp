...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Quad_Normal = NULL;
...
HRESULT initialize()
{
	...
	const char* pixelShaderSourceCode =
		...
		"if(lPixelKeyPressed == 1)" \
		"{" \
		"float3 normTransposeNormal = normalize(output.transformedNormal);" \
		"float3 normSource_light_direction = normalize(output.lightDirection);" \
		"float3 normviewerVector = normalize(output.viewVector);" \
		...
		"phong_ads_light_per_pixel = ambient + diffuse + specular;" \
		"}" \
		"else" \
		"{" \
			"float4 texColor = myTexture2D.Sample(mySamplerState, output.texcoord);" \
			"output.phong_ads_light = output.color * texColor * output.phong_ads_light;" \
		...
		"}" \
			"float4 texColor = myTexture2D.Sample(mySamplerState, output.texcoord);" \
			"phong_ads_light_per_pixel = output.color * texColor * phong_ads_light_per_pixel;" \
	...
	inputElementDesc[1].SemanticName = "COLOR"; // need same in shader
	...
	inputElementDesc[2].SemanticName = "NORMAL"; // need same in shader
	inputElementDesc[2].SemanticIndex = 0;
	inputElementDesc[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementDesc[2].InputSlot = 2;	// amc_clr
	...
	inputElementDesc[3].SemanticName = "TEXCOORD"; // need same in shader
	inputElementDesc[3].SemanticIndex = 0;
	inputElementDesc[3].Format = DXGI_FORMAT_R32G32_FLOAT;
	inputElementDesc[3].InputSlot = 3;	// amc_clr
	...
	hr = LoadD3DTexture(L"marble.bmp", &gpID3D11ShaderResourceView_Quad_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
...
void display()
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Quad, &stride, &offset);

	stride = sizeof(float) * 11;
	offset = sizeof(float) * 6;
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_VertexBuffer_Quad, &stride, &offset);

	stride = sizeof(float) * 11;
	offset = sizeof(float) * 9;
	gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_VertexBuffer_Quad, &stride, &offset);
