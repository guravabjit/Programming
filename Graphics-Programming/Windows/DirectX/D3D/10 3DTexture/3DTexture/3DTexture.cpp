...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Triangle_Texture = NULL;
...
ID3D11ShaderResourceView* gpID3D11ShaderResourceView_Triangle_Texture = NULL;
ID3D11SamplerState* gpID3D11SamplerState_Triangle_Texture = NULL;
...
HRESULT initialize()
{
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(triangleTexCord);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Triangle_Texture);

	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "ID3D11Device::CreateBuffer() Failed For Vertex Buffer for triangle texture.\n");
		fclose(gpLogFile);
		return(hr);
	}
	else
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle_Texture,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresource);
	memcpy(mappedSubresource.pData, triangleTexCord, sizeof(triangleTexCord));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Triangle_Texture, NULL);
	...
	CoInitialize(NULL);
	hr = LoadD3DTexture(L"stone.bmp", &gpID3D11ShaderResourceView_Triangle_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "LoadD3DTexture() Failed For triangle Texture.\n ");
		fclose(gpLogFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "LoadD3DTexture() Succeeded For triangle Texture.\n ");
		fclose(gpLogFile);
	}
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc, &gpID3D11SamplerState_Triangle_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "ID3D11Device::CreateSamplerState() Failed For triangle Texture.\n");
		fclose(gpLogFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
		fprintf_s(gpLogFile, "ID3D11Device::CreateSamplerState() Succeeded For triangle Texture.\n ");
		fclose(gpLogFile);
	}


	hr = LoadD3DTexture(L"kundali.bmp", &gpID3D11ShaderResourceView_Quad_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLogFile, gszLogFileName, "a+");
...
void display()
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Triangle_Texture, &stride, &offset);

	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_Triangle_Texture);
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Triangle_Texture);
	...
	translationMatrix = XMMatrixTranslation(-2.0f, 0.0f, 7.0f);
