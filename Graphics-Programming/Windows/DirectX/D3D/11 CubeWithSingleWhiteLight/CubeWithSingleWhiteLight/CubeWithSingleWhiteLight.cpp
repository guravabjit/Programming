...
HRESULT initialize()
{
	...
	const char* vertexShaderSourceCode =
		...
		"{" \
			...
			"{" \
				...
				"float3 transposeNormal = normalize(mul((float3x3)worldViewMatrix, (float3)normal));" \
				"float3 source_light_direction = (float3)normalize(lightPosition - eyeCoordinates);" \
				"output.outDiffuseLight = ld * kd * dot(source_light_direction, transposeNormal);" \
			"}" \
			"else" \
			"{" \
				"output.outDiffuseLight = float4(1.0, 1.0, 1.0, 1.0);" \
			"}" \
	...
	const char* pixelShaderSourceCode =
		"float4 main(float4 position: SV_POSITION, float4 outDiffuseLight: COLOR) : SV_TARGET" \
		"{" \
		"return(outDiffuseLight);" \
		"}";
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(quadColor);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Quad_Normal);
	...
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Quad_Normal,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresource);
	memcpy(mappedSubresource.pData, quadColor, sizeof(quadColor));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Quad_Normal, NULL);
...
void display()
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Quad_Normal, &stride, &offset);
