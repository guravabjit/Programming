...
void display()
{
	...
	glLineWidth(3.0f);
	for (float fItr = -1.0f; fItr <= 1.0f; fItr += fDiff)
	{
		glBegin(GL_LINES);
	...
	// instead of checking each time condition of 0 value in loop, overlapping center horizontal line
	glBegin(GL_LINES);
	glColor3f(1.0f, 0.0f, 0.0f);
