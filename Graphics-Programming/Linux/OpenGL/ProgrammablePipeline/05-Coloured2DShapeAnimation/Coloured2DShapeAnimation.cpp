...
void initialize()
{
	...
	glXMakeCurrent(gpDisplay, gWindow, gGlxContext);
       ...
       // Error code checking function by AG
		...
		const GLfloat quadColorVertices[] =
		{
			0, 0, 1,
			0, 0, 1,
			0, 0, 1,
...
void display(void)
{
	...
	modelViewMatrix = modelViewMatrix * rotationMatrix;//(angleTriangle, 0.0f, 1.0f, 0.0f);
		printf("angleTriangle:%f\n",angleTriangle);
