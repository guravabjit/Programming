...
float gLightDiffuseV[] = { 1.0f, 1.0f, 1.0f, 1.0f };
float gLightSpecularV[] = { 1.0f, 1.0f, 1.0f, 1.0f };
float gLightPositionV[] = { 100.0f, 100.0f, 100.0f, 1.0f };
...
void initPerVertexLight()
{
	...
	"#version 450 core" \
		...
		"\n" \
				/*"mat3 normalMatrix = mat3(transpose(inverse(u_mv_matrixV)));" \*/
				/*"mat3 normalMatrix = mat3(u_mv_matrixV);" \*/
				"vec3 transposeNormal = normalize(mat3(u_v_matrixV * u_m_matrixV) * vNormal);" \
		"\n" \
				/*"vec3 source_light_direction = vec3(u_light_positionV.x - eyeCoordinates.x, u_light_positionV.y - eyeCoordinates.y, u_light_positionV.z - eyeCoordinates.z);" \*/
