...
/*	printf("fTransIX : %f\n",fTransIX);
	...
	if (true == bDrawPlane)
	{
		...
		fTransPlaneTopX += 0.004f;

		if (fTransPlaneTopY >= 0.0f)
		{
			fTransPlaneTopY -= 0.004f;
		}
		...
		fTransPlaneMiddleX += 0.004f;
		//fTransPlaneMiddleY += 0.01f;

		fTransPlaneBottomX += 0.004f;

		if (fTransPlaneBottomY <= 0.0f)
		{
			fTransPlaneBottomY += 0.004f;
		}
	...
	if (bDrawD == true)
	{
		...
		{
			fOrangeColorX += 0.001f;
		}

		if (fOrangeColorY < 0.5f)
		{
			fOrangeColorY += 0.001f;
		}
