...
void display(void)
{
	...
	glPushMatrix();
	glRotatef((GLfloat)giSholder, 0.0f, 0.0f, 1.0f);
	...
	if (NULL != gpQuadric)
	{
		gluSphere(gpQuadric, 0.5f, 10, 10);
		gluDeleteQuadric(gpQuadric);
		gpQuadric = NULL;
	}
	else
	{
		fprintf(gpLogFile, "gpQuadric is NULL for sholder");
	}
	glPopMatrix();
	...
	//glPushMatrix(); // TalHat Kadhanyasathi
	...
	{
		fprintf(gpLogFile, "gpQuadric is NULL for elbow");
	}
	glPopMatrix();
	glPopMatrix();
