...
void initialize()
{
	GLuint LoadTexture(GLuint &Texture, char*);
	void uninitialize();
	void resize(int, int);
...
"#version 450 core" \
		...
		"{"	\
			...
			"{" \
				...
				"fragColor = texture(u_sampler, out_TexCord) * vec4(out_fong_ads_light, 1.0);" \
			"}" \
			"else" \
			"{" \
	...
	LoadTexture(gTextureSun, (char*)"./sun.bmp");
	LoadTexture(gTextureEarth,(char*)"./earth.bmp");
	LoadTexture(gTextureMoon,(char*)"./moon.bmp");
...
GLuint LoadTexture(GLuint &Texture, char* path)
{
