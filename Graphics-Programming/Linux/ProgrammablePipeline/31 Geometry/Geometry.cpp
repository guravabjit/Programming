...
void initialize()
{
	...
	GLuint gGlGeometryShaderObject;
	...
	gGlGeometryShaderObject = glCreateShader(GL_GEOMETRY_SHADER);

	// write vertex shader code
	const GLchar *pszGeometryShaderSourceCode =
		"#version 450 core" \
		"\n" \
		"layout(triangles)in;" \
	...
	// specify above shader source code to vertex shader object
	glShaderSource(gGlGeometryShaderObject,  // shader object
		1, // number of strings
		(const GLchar**)&pszGeometryShaderSourceCode, // array of strings
		NULL // array of strings length
	);

	// compile vertex shader
	glCompileShader(gGlGeometryShaderObject);

	// Error code checking function by AG
	memset(szglErrorStr, 0, ERROR_SIZE);
	strcpy(szglErrorStr,  "Error in vertex shader object code : ");
	bErrorStatus = GetGlErrorStatus(enGlGET_Shader, gGlGeometryShaderObject, szglErrorStr);
	if (GL_FALSE == bErrorStatus)
	{
		fprintf(gpLogFile, "%s.\n", szglErrorStr);
		return ;
	...
	glAttachShader(gGlShaderProgramObject, gGlGeometryShaderObject);
