...
#define NUMBER_OF_VERTICES 6000
#define PI 3.14
...
void display(void)
{
	...
	for (int iItr = 20; iItr >= 0; iItr--)
	{
		...
		if (iItr >= 10)
		{
			...
			glVertex3f(-1.0f, fIndex, 0.0f);
			glVertex3f(1.0f, fIndex, 0.0f);

			glColor3f(0.0f, 1.0f, 0.0f);
			glVertex3f(fIndex, -1.0f, 0.0f);
			glVertex3f(fIndex, 1.0f, 0.0f);
			fIndex += 1.0f / 10.0f;
		}
		else
	...
	glVertex3f(0.0f, fTopx/2.0f, 0.0f);
	glVertex3f(fTopx-1.0f, fTopx -1.0f, 0.0f);
	glVertex3f(1.0f - fTopx, -(1.0f-fTopx), 0.0f);
	glEnd();*/
	...
	for (float fItr = -1.0f; fItr <= 1.05f; fItr += fDiff)
	{
		...
		glVertex3f(-1.0f, fItr, 0.0f);
		glVertex3f(1.0f, fItr, 0.0f);

		glVertex3f(fItr, -1.0f, 0.0f);
		glVertex3f(fItr, 1.0f, 0.0f);
	...
	{
		glVertex3f((GLfloat)cos((5.0f * PI * iItr) / NUMBER_OF_VERTICES), (GLfloat)sin((5.0f * PI * iItr) / NUMBER_OF_VERTICES), 0.0f);
		iItr++;
		glVertex3f((GLfloat)cos((5.0f * PI * iItr) / NUMBER_OF_VERTICES), (GLfloat)sin((5.0f * PI * iItr) / NUMBER_OF_VERTICES), 0.0f);
	}
